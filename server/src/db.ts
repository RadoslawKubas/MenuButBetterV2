...
/** Pula TYLKO gdy DB jest gotowa (po initDb). null = brak DB / nie zainicjowano. Dla modułów obok db.ts. */
export function getReadyPool(): Pool | null {
  return ready ? getPool() : null;
}
...
export async function getStats(): Promise<Stats> {
  ...
  const [scans, dishes, totals, byModel, byOp, byDay, errors, recentErrors, since] = await Promise.all([
    ...
    p.query(`SELECT to_char(date_trunc('day', created_at),'YYYY-MM-DD') AS day,
             count(*) FILTER (WHERE type='scan')::int AS scans, coalesce(sum(cost_usd),0) AS cost
  ...
  return {
    ...
    byDay: byDay.rows.map((r) => ({ day: r.day, scans: r.scans, cost: Number(r.cost) })),
